...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Tableau de bord admin TikTok Task Partners." },
    ],
  }),
  component: AdminDashboard,
});

type AdminVideo = { id: number; title: string; url: string; category: string; reward: number; status: "Actif" | "Inactif" };
type Withdrawal = { id: number; user: string; amount: number; method: string; data: string; status: "En attente" | "Approuvé" | "Rejeté" };

const seedVideos: AdminVideo[] = [
  { id: 1, title: "Beauty Launch Audit", url: "https://www.tiktok.com/embed/demo-1", category: "Beauty", reward: 15, status: "Actif" },
  { id: 2, title: "Tech Demo Review", url: "https://www.tiktok.com/embed/demo-2", category: "Technology", reward: 18, status: "Actif" },
];

const seedWithdrawals: Withdrawal[] = [
  { id: 1001, user: "Mia Carter", amount: 1000, method: "PayPal Email", data: "mia@example.com", status: "En attente" },
  { id: 1002, user: "Noah Brooks", amount: 1240, method: "Cash App", data: "$noahreviews", status: "En attente" },
];
...
function AdminDashboard() {
  ...
  function addVideo(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setVideos((value) => [{ id: Date.now(), title, url, category, reward: Math.min(20, Math.max(5, reward)), status: "Actif" }, ...value]);
    setTitle("");
    setUrl("");
    setCategory("");
    setReward(15);
      ...
      <main className="grid min-h-dvh place-items-center bg-[#070B14] px-4 text-white">
        <form onSubmit={login} className="w-full max-w-[390px] rounded-[8px] border border-white/10 bg-[#101827] p-5 shadow-2xl">
          <div className="mb-5 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-black">Tableau de bord admin</h1>
              <p className="text-sm text-slate-400">Accès admin protégé par rôle</p>
            </div>
          </div>
          ...
          <input className="mb-4 h-12 w-full rounded-[8px] bg-white px-3 text-sm font-bold text-black outline-none" placeholder="Mot de passe" type="password" required />
    ...
    <main className="min-h-dvh bg-[#070B14] px-4 py-5 text-white">
      <section className="mx-auto w-full max-w-6xl">
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-4">
          <Stat icon={<UsersRound />} label="Utilisateurs" value="1,248" />
          <Stat icon={<Film />} label="Vidéos" value={String(videos.length)} />
          <Stat icon={<DollarSign />} label="Soldes totaux" value={money(totalBalances)} />
          <Stat icon={<CheckCircle2 />} label="Avis" value="8,904" />
        </div>

        <div className="grid gap-5 lg:grid-cols-[1fr_380px]">
          <section className="rounded-[8px] border border-white/10 bg-[#101827] p-4">
            <h2 className="mb-4 text-xl font-black">Gestion des vidéos</h2>
            <form onSubmit={addVideo} className="mb-4 grid gap-2 sm:grid-cols-5">
              <input className="h-11 rounded-[8px] bg-white px-3 text-sm font-bold text-black" placeholder="Titre" value={title} onChange={(event) => setTitle(event.target.value)} required />
          ...
          <aside className="space-y-5">
            <section className="rounded-[8px] border border-white/10 bg-[#101827] p-4">
              <h2 className="mb-3 text-xl font-black">Contrôle du système</h2>
              <label className="text-sm font-bold text-slate-300">Limite quotidienne d’avis</label>
              ...
              <p className="mt-2 text-xs text-slate-400">Plage autorisée : 5 à 10 vidéos par utilisateur et par jour.</p>
            </section>

            <section className="rounded-[8px] border border-white/10 bg-[#101827] p-4">
              <h2 className="mb-3 text-xl font-black">Modération des retraits</h2>
              <div className="space-y-2">
                {withdrawals.map((item) => (
                  <div className="rounded-[8px] bg-white/5 p-3 text-sm" key={item.id}>
                    ...
                    <div className="mt-3 flex gap-2">
                      <button className="flex h-9 flex-1 items-center justify-center gap-1 rounded-[8px] bg-emerald-500 font-black" onClick={() => setWithdrawalStatus(item.id, "Approuvé")} type="button"><CheckCircle2 size={15} />Approuver</button>
                      <button className="flex h-9 flex-1 items-center justify-center gap-1 rounded-[8px] bg-rose-500 font-black" onClick={() => setWithdrawalStatus(item.id, "Rejeté")} type="button"><XCircle size={15} />Rejeter</button>
                    </div>
                  </div>
                ))}
              </div>
